...
correlation_matrix_pearson <- cor(SUBSET_Teacher, use = "pairwise.complete.obs")
num_vars <- nrow(correlation_matrix_pearson)
var_names <- c("TC_reading", "TC_writing", "TC_comprehension", 
               "TC_maths", "TC_imagin_creat", "TC_oral_comm", "TC_prob_solving")
formatted_correlation <- list()
for(i in 1:num_vars) {
  for(j in 1:num_vars) {
    var1 <- var_names[i]
    var2 <- var_names[j]
    correlation <- correlation_matrix_pearson[i, j]
    formatted_string <- paste(var1, "*", var2, "=", round(correlation, 4))
    formatted_correlation <- c(formatted_correlation, list(formatted_string))
  }
}

for(correlation_info in formatted_correlation) {
  print(correlation_info)
}
correlation_df <- data.frame(Correlation = unlist(formatted_correlation))
correlation_df <- correlation_df[order(-correlation_matrix_pearson), , drop = FALSE]
print(correlation_df)

mosaicplot(table(SUBSET_Teacher), main = "Mosaic Plot of Categorical Variables")
heatmap(correlation_matrix_pearson, main = "Heatmap of Pearson Correlations")

library(igraph)
dissimilarity_matrix_pearson <- 1 - correlation_matrix_pearson
graph_pearson <- graph_from_adjacency_matrix(as.matrix(dissimilarity_matrix_pearson), weighted = TRUE, mode = "undirected")
V(graph_pearson)$name <- rownames(correlation_matrix_pearson)
plot(graph_pearson, layout = layout_with_fr(graph_pearson), vertex.label.dist = 2, vertex.size = 10, vertex.label.cex = 0.8, edge.width = E(graph_pearson)$weight, main = "Ability Network Visualization")

library(igraph)
library(rgl)

plot_graph_3d <- function(graph, main_title) {
  rgl.open()
  plot3d(layout_with_fr(graph),
         vertex.label=V(graph)$name,
         edge.width=E(graph)$weight,
         box=FALSE,
         main=main_title)
}
plot_graph_3d(graph_pearson, "Pearson Ability Network Visualization")
...
# Function to plot the graph in 3D
plot_graph_3d <- function(graph, main_title) {
  # Use Fruchterman-Reingold layout algorithm to generate 3D coordinates
  layout <- layout_with_fr(graph, dim = 3)
  ...
  vertices <- layout
  points3d(vertices, col = "blue", size = 5)
  
  edges <- as.data.frame(get.edgelist(graph))
  for (i in 1:nrow(edges)) {
    lines3d(rbind(vertices[edges[i, 1], ], vertices[edges[i, 2], ]), col = "blue")
  }
}

# Call the function to plot the graph in 3D
plot_graph_3d(graph_pearson, "Ability Network Visualization (3D)")



# Spearman 

if (!is.data.frame(SUBSET_Teacher)) {
  SUBSET_Teacher <- as.data.frame(SUBSET_Teacher)
}
sum(is.na(SUBSET_Teacher))
any(is.nan(as.matrix(SUBSET_Teacher)))
any(is.infinite(as.matrix(SUBSET_Teacher)))
SUBSET_Teacher <- na.omit(SUBSET_Teacher)
dim(SUBSET_Teacher)
correlation_matrix_spearman <- cor(SUBSET_Teacher, method = "spearman")

heatmap(correlation_matrix_spearman, main = "Heatmap of Spearman Correlations")


library(igraph)
dissimilarity_matrix_spearman <- 1 - correlation_matrix_spearman
graph_spearman <- graph_from_adjacency_matrix(as.matrix(dissimilarity_matrix_spearman), weighted = TRUE, mode = "undirected")
V(graph_spearman)$name <- rownames(correlation_matrix_spearman)
plot(graph_spearman, layout = layout_with_fr(graph_spearman), vertex.label.dist = 2, vertex.size = 10, vertex.label.cex = 0.8, edge.width = E(graph_spearman)$weight, main = "Ability Network Visualization")
...
# Function to plot the graph in 2D
plot_graph_2d <- function(graph, main_title) {
  plot(graph, 
...
# Plot the Pearson graph
plot_graph_2d(graph_pearson, "Pearson Ability Network Visualization")

# Plot the Spearman graph
plot_graph_2d(graph_spearman, "Spearman Ability Network Visualization")

print(correlation_matrix_pearson)
print(correlation_matrix_spearman)

# Function to plot the graph in 2D with edges of different colors
plot_graph_2d <- function(graph, main_title) {
  # Get edge weights or any other edge attribute for color assignment
  edge_colors <- ifelse(E(graph)$weight > mean(E(graph)$weight), "red", "blue")  # Example: Red for above average weight, blue for below
  
  # Plot the graph with different edge colors
  plot(graph, 
       layout = layout_with_fr(graph),
       vertex.label.dist = 2,
       vertex.size = 10,
       vertex.label.cex = 0.8,
       edge.color = edge_colors,  # Assign different colors to edges
       main = main_title)
}
...
# Plot the Pearson graph with edges of different colors
plot_graph_2d(graph_pearson, "Pearson Ability Network Visualization")

# Plot the Spearman graph with edges of different colors
plot_graph_2d(graph_spearman, "Spearman Ability Network Visualization")

# Calculate the mean edge weight for the Pearson graph
mean_weight_pearson <- mean(E(graph_pearson)$weight)

# Calculate the mean edge weight for the Spearman graph
mean_weight_spearman <- mean(E(graph_spearman)$weight)

# Print the mean edge weights
cat("Mean edge weight for Pearson graph:", mean_weight_pearson, "\n")
cat("Mean edge weight for Spearman graph:", mean_weight_spearman, "\n")

install.packages("MASS")
# Load required library for MDS
library(MASS) 

# Compute Multidimensional Scaling (MDS) using Pearson correlation matrix
mds_pearson <- cmdscale(correlation_matrix_pearson)

# Compute Multidimensional Scaling (MDS) using Spearman correlation matrix
mds_spearman <- cmdscale(correlation_matrix_spearman)

# Plot the results for Pearson correlation matrix
plot(mds_pearson, main = "MDS Plot (Pearson Correlation)", 
     xlab = "Dimension 1", ylab = "Dimension 2", pch = 19, col = "blue")

# Add labels for each point (ability)
text(mds_pearson, labels = rownames(correlation_matrix_pearson), pos = 3)

# Plot the results for Spearman correlation matrix
plot(mds_spearman, main = "MDS Plot (Spearman Correlation)", 
     xlab = "Dimension 1", ylab = "Dimension 2", pch = 19, col = "red")

# Add labels for each point (ability)
text(mds_spearman, labels = rownames(correlation_matrix_spearman), pos = 3)
...
dissimilarity_matrix <- 1 - correlation_matrix_pearson

# Convert dissimilarity matrix to a graph
graph <- graph_from_adjacency_matrix(as.matrix(dissimilarity_matrix), weighted = TRUE, mode = "undirected")

V(graph)$name <- rownames(correlation_matrix_pearson)

# Plot the network in 3D
rgl.open()
plot3d(layout_with_fr(graph), vertex.label=V(graph)$name, edge.width=E(graph)$weight)

# Plot the network
plot(graph, layout = layout_with_fr(graph), vertex.label.dist = 2, vertex.size = 10, vertex.label.cex = 0.8, edge.width = E(graph)$weight, main = "Ability Network Visualization")
